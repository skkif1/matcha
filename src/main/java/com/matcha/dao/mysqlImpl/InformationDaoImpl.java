...
public class InformationDaoImpl implements InformationDao {
    ...
    public void savePhoto(MultipartFile[] photos, Integer userId) throws IOException {
        ...
        for (MultipartFile photo : photos) {
                ...
                System.out.println(CDN_WEB_ADDRESS + userId + "/" + photo.getOriginalFilename());
            }
    }
    ...
    public List<User> getUserConnections(Integer userId) {
        ...
        String sql = "SELECT * FROM user" +
                " INNER JOIN matches " +
                " ON (user.id = matches.connection_author_id OR user.id = matches.user2_id) AND user.id != ? AND (matches.connection_author_id = ? OR matches.user2_id = ?)" +
                " LIMIT 20";
    ...
    public void getNewEventsNumber(Map<String, Integer> eventsHolder, Integer userId) {
        ...
        {
            eventsHolder.put("likes", rs.getInt(1));
            eventsHolder.put("connections", rs.getInt(2));
            eventsHolder.put("visitors", rs.getInt(3));
            return eventsHolder;
        });
    }
