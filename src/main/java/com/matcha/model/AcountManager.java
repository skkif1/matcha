...
public class AcountManager {
    ...
    public List<User> searchForUsers(SearchRequest searchParams, HttpSession session) {
        ...
        if (foundUsers.contains(userWhoSearch))
            foundUsers.remove(userWhoSearch);
        return foundUsers;
    }
    ...
    public List<User> filterSuggestedUsers(List<User> suggestedUsers, User userWhoSearch, SearchRequest request) {
        ...
        res = res.stream().filter(user ->
        {
            return checkIfUserEligableForSearch(user);
        }).collect(Collectors.toList());
    ...
    private void removeUsersWithoutRights(List<User> listOfUsers) {
        if (listOfUsers != null)
            listOfUsers.removeIf(user -> !checkIfUserEligableForSearch(user));
    }
}
