...
public class AcountController {
    ...
    public String getAcount(@PathVariable("id") Integer id, HttpSession session) throws IllegalAccessException {
        User visitor = (User) session.getAttribute("user");
        User user = new User();
        user.setId(id);

        if (id == visitor.getId() || !acountManager.checkIfUserEligableForSearch(user))
            return "redirect:/";
    ...
    public @ResponseBody
    String likeUser(@PathVariable("userId") Integer userId, HttpSession session) {
    ...
    public @ResponseBody
    String dislikeUser(@PathVariable("userId") Integer authorId, HttpSession session) {
    ...
    public @ResponseBody
    String addToBlackList(@PathVariable("userId") Integer userId, HttpSession session) {
    ...
    public @ResponseBody
    String checkConnection(@PathVariable("userId") Integer userToCheck, HttpSession session) {
